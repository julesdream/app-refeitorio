...
const cardapioSemana = [
  {
    id: 1, diaSemana: "Segunda-feira", data: "25/05",
    refeicoes: [
      { tipo: "ALMOÇO", icone: "wb-sunny" as const, itens: ["Arroz branco / parboilizado", "Feijão preto", "Iscas de carne bovina", "Polenta frita", "Salada de alface e tomate"] },
      { tipo: "JANTAR", icone: "nights-stay" as const, itens: ["Arroz branco / parboilizado", "Feijão preto", "Cachorro quente", "Batata palha", "Salada mista"] },
    ],
  },
  {
    id: 2, diaSemana: "Terça-feira", data: "26/05",
    refeicoes: [
      { tipo: "ALMOÇO", icone: "wb-sunny" as const, itens: ["Arroz branco / parboilizado", "Feijão carioca", "Filé de frango grelhado", "Creme de milho", "Salada de cenoura e beterraba"] },
      { tipo: "JANTAR", icone: "nights-stay" as const, itens: ["Arroz branco / parboilizado", "Feijão carioca", "Carne de panela com batatas", "Farofa", "Salada verde"] },
    ],
  },
  {
    id: 3, diaSemana: "Quarta-feira", data: "27/05",
    refeicoes: [
      { tipo: "ALMOÇO", icone: "wb-sunny" as const, itens: ["Arroz branco / parboilizado", "Feijão preto", "Strogonoff de frango", "Batata palha", "Salada de repolho"] },
      { tipo: "JANTAR", icone: "nights-stay" as const, itens: ["Sopa de agnolini", "Pão francês", "Queijo ralado", "Fruta da estação"] },
    ],
  },
  {
    id: 4, diaSemana: "Quinta-feira", data: "28/05",
    refeicoes: [
      { tipo: "ALMOÇO", icone: "wb-sunny" as const, itens: ["Arroz branco / parboilizado", "Lentilha", "Bife suíno acebolado", "Massa alho e óleo", "Salada de tomate"] },
      { tipo: "JANTAR", icone: "nights-stay" as const, itens: ["Arroz de forno com frios", "Salada mista", "Ovos cozidos", "Fruta da estação"] },
    ],
  },
  {
    id: 5, diaSemana: "Sexta-feira", data: "29/05",
    refeicoes: [
      { tipo: "ALMOÇO", icone: "wb-sunny" as const, itens: ["Arroz branco / parboilizado", "Feijão preto", "Peixe empanado", "Purê de batatas", "Salada de folhas verdes"] },
      { tipo: "JANTAR", icone: "nights-stay" as const, itens: ["Pizza de calabresa e queijo", "Suco natural", "Sobremesa doce"] },
    ],
  },
];
      ...
      <View style={s.cardInner}>
        ...
        {itens.map((item, i) => (
          <View key={i} style={s.itemRow}>
            ...
            <Text style={s.itemText} numberOfLines={2}>{item}</Text>
          </View>
        ))}
      </View>
    </View>
...
export default function CardapioScreen() {
  ...
  const Header = (
    <View style={[s.header, isDesktop && { paddingHorizontal: PADDING_H_DSK }]}>
      <View>
        ...
        <Text style={s.welcome}>{user?.login.split("@")[0] ?? "estudante"}</Text>
      </View>
      <TouchableOpacity style={s.logoutBtn} onPress={lidarComLogout} activeOpacity={0.8}>
  ...
  const Titulo = (
    <View style={[s.titleRow, isDesktop && { paddingHorizontal: PADDING_H_DSK }]}>
  ...
  if (isDesktop) {
    return (
      <ScrollView style={s.container} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={[s.grid, { paddingHorizontal: PADDING_H_DSK, gap: COL_GAP_DSK, marginTop: 8 }]}>
    ...
    <View style={s.container}>
      ...
      >
        {cardapioSemana.map((dia, idx) => {
          ...
          return (
            <TouchableOpacity
              key={dia.id}
              style={[s.diaBtn, isAtivo && s.diaBtnAtivo, isHoje && !isAtivo && s.diaBtnHoje]}
              onPress={() => irParaDia(idx)}
              activeOpacity={0.75}
            >
              <Text style={[s.diaBtnData, isAtivo && s.diaBtnDataAtiva]}>{dia.data}</Text>
      ...
      >
        {cardapioSemana.map((dia) => {
          ...
          return (
            <ScrollView 
              key={dia.id} 
              style={[s.diaCol, { width: CARD_W_MOB, marginRight: CARD_GAP_MOB }]}
              showsVerticalScrollIndicator={false}
              contentContainerStyle={{ paddingBottom: 24 }}
            >
              <View style={[s.diaHeaderRow, isHoje && s.diaHeaderHoje]}>
                <View>
                  <Text style={[s.diaNome, isHoje && { color: VERDE_MID }]}>{dia.diaSemana}</Text>
      ...
      <View style={s.paginacao}>
        {cardapioSemana.map((_, idx) => (
          <TouchableOpacity key={idx} onPress={() => irParaDia(idx)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
...
const s = StyleSheet.create({
  ...
  header: {
    ...
    paddingHorizontal: PADDING_H_MOB, marginBottom: 20,
  },
  saudacao: { fontSize: 13, color: TEXTO_SUAVE, fontWeight: "500" },
  welcome: { fontSize: 22, fontWeight: "800", color: TEXTO_FORTE, textTransform: "capitalize" },
  logoutBtn: {
    backgroundColor: "#c62828", flexDirection: "row", alignItems: "center",
    paddingHorizontal: 14, paddingVertical: 9, borderRadius: 12, gap: 6,
  ...
  titleIconWrap: {
    width: 38, height: 38, borderRadius: 10, backgroundColor: VERDE_MID,
    alignItems: "center", justifyContent: "center",
    shadowColor: VERDE_MID, shadowOffset: { width: 0, height: 3 },
    shadowOpacity: 0.35, shadowRadius: 5, elevation: 4,
  },
  title: { fontSize: 17, fontWeight: "800", color: VERDE_DARK, letterSpacing: -0.3 },
  subtitle: { fontSize: 12, color: TEXTO_SUAVE, marginTop: 1 },

  // ── Desktop grid
  grid: { flexDirection: "row" },
  gridCol: { flexShrink: 1 },
  colHeader: { alignItems: "center", marginBottom: 12, paddingVertical: 6, minHeight: 70 },
  colDia: { fontSize: 13, fontWeight: "800", color: TEXTO_MED, textAlign: "center" },
  colData: { fontSize: 11, color: TEXTO_SUAVE, marginTop: 2 },

  // ── Mobile nav
  diasNav: {
    paddingHorizontal: PADDING_H_MOB, gap: 8, marginBottom: 16, alignItems: "flex-start",
  },
  diaBtn: {
    alignItems: "center", paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12,
    backgroundColor: CINZA_CARD, borderWidth: 1, borderColor: CINZA_BORDA, minWidth: 52,
  },
  diaBtnAtivo: {
    backgroundColor: VERDE_MID, borderColor: VERDE_MID,
    shadowColor: VERDE_MID, shadowOffset: { width: 0, height: 3 },
  ...
  diaBtnNome: { fontSize: 11, fontWeight: "800", color: TEXTO_MED, marginTop: 1 },
  diaBtnNomeAtivo: { color: "#fff" },
  dotHoje: { width: 5, height: 5, borderRadius: 3, backgroundColor: VERDE_LIGHT, marginTop: 3 },

  // ── Mobile colunas
  diaCol: {},
  diaHeaderRow: {
    flexDirection: "row", justifyContent: "space-between", alignItems: "center",
    marginBottom: 10, paddingHorizontal: 2, height: MOB_DAY_HEADER_H,
  },
  diaHeaderHoje: {},
  ...
  badgeHoje: {
    ...
    backgroundColor: VERDE_LIGHT, paddingHorizontal: 9, paddingVertical: 5, borderRadius: 10,
    shadowColor: VERDE_LIGHT, shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.3, shadowRadius: 4, elevation: 3,
  },
  badgeHojeText: { color: "#fff", fontSize: 10, fontWeight: "800", letterSpacing: 0.5 },

  // ── Card
  card: {
    borderRadius: 12, overflow: "hidden",
    backgroundColor: CINZA_CARD, shadowColor: "#000",
  ...
  itemRow: { flexDirection: "row", alignItems: "flex-start", gap: 7, marginBottom: 4 },
  ...
  dotAtivo: { width: 20, height: 6, borderRadius: 3, backgroundColor: VERDE_MID },
});